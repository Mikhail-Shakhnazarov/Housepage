...
def _check_body(check: Check) -> dict[str, Any]:
    return {
        ...
        "trigger_answer": check.trigger_answer.value if check.trigger_answer is not None else None,
    }
...
class SQLiteHouseholdStore:
    ...
    def _ensure_column(
        ...
        if name not in SQLiteHouseholdStore._columns(connection, table):
    ...
    def _create_tables(connection: sqlite3.Connection) -> None:
        connection.executescript(
            ...
            CREATE TABLE IF NOT EXISTS checks(
                ...
                trigger_answer TEXT CHECK(trigger_answer IS NULL OR trigger_answer IN ('yes','no'))
            );
    ...
    @classmethod
    def _upgrade_additive_schema(cls, connection: sqlite3.Connection) -> None:
        cls._ensure_column(connection, "checks", "trigger_answer", "TEXT")
        event_columns = {
            "event_id": "TEXT",
            "session_id": "TEXT",
            "device_id": "TEXT",
        ...
        for name, declaration in event_columns.items():
            cls._ensure_column(connection, "events", name, declaration)
    ...
    @classmethod
    def _seed_definition_revisions(cls, connection: sqlite3.Connection, *, actor: str) -> None:
        if connection.execute("SELECT 1 FROM definition_revisions LIMIT 1").fetchone():
            return
        ...
        for row in connection.execute("SELECT id,name,sort_order FROM rooms ORDER BY id"):
            ...
            cls._insert_revision(connection, "room", str(row["id"]), 1, body, actor, updated_at)
        for row in connection.execute(
            "SELECT id,room,prompt,linked_task_ids_json,trigger_answer FROM checks ORDER BY id"
        ):
            body = {
                "id": row["id"],
                "room": row["room"],
            ...
            cls._insert_revision(connection, "check", str(row["id"]), 1, body, actor, updated_at)
        for row in connection.execute(
            "SELECT id,title,room,kind,effort,minutes_est,frequency_days FROM tasks ORDER BY id"
        ):
            body = dict(row)
            cls._insert_revision(connection, "task", str(row["id"]), 1, body, actor, updated_at)
    ...
    def _migrate(self) -> None:
        with self._connect() as connection:
            ...
            if version not in {"1", "2", SCHEMA_VERSION}:
                raise StoreError(f"unsupported household store schema {version}")
            if version != SCHEMA_VERSION:
                self._upgrade_additive_schema(connection)
                self._seed_definition_revisions(
                    connection, actor=f"migration:v{version}-v{SCHEMA_VERSION}"
                )
                connection.execute(
    ...
    def _current_revision_row(
        connection: sqlite3.Connection, entity_kind: str, entity_id: str
    ) -> sqlite3.Row | None:
        return connection.execute(
            """
            SELECT version,body_sha256 FROM definition_revisions
            WHERE entity_kind=? AND entity_id=? ORDER BY version DESC LIMIT 1
            """,
        ...
        *,
        ...
        with self._connect() as connection:
            ...
            if entity_kind == "room":
                ...
                connection.execute(
                    """
                    INSERT INTO checks(id,room,prompt,linked_task_ids_json,trigger_answer)
                    VALUES(?,?,?,?,?)
                    ON CONFLICT(id) DO UPDATE SET
                        room=excluded.room,prompt=excluded.prompt,
                        linked_task_ids_json=excluded.linked_task_ids_json,
                        trigger_answer=excluded.trigger_answer
                    """,
                    (
                        body["id"],
                        body["room"],
                ...
                connection.execute(
                    ...
                    """,
                    (
                        body["id"],body["title"],body["room"],body["kind"],
                        body["effort"],body["minutes_est"],body["frequency_days"],
                    ),
                )
            else:
            ...
            return self._insert_revision(
                connection, entity_kind, entity_id, version, body, actor, updated_at.isoformat()
            )

    def put_room(self, room: RoomDefinition) -> str:
        return self._write_definition(
            entity_kind="room", entity_id=room.id, body=_room_body(room), expected_hash=None,
            actor="system:initial", updated_at=datetime.now(timezone.utc)
        )

    def put_check(self, check: Check) -> str:
        return self._write_definition(
            entity_kind="check", entity_id=check.id, body=_check_body(check), expected_hash=None,
            actor="system:initial", updated_at=datetime.now(timezone.utc)
        )

    def put_task(self, task: Task) -> str:
        return self._write_definition(
            entity_kind="task", entity_id=task.id, body=_task_body(task), expected_hash=None,
            actor="system:initial", updated_at=datetime.now(timezone.utc)
        )
    ...
    def revise_room(
        ...
        return self._write_definition(
            entity_kind="room", entity_id=room.id, body=_room_body(room), expected_hash=expected_hash,
    ...
    def revise_check(
        ...
        return self._write_definition(
            entity_kind="check", entity_id=check.id, body=_check_body(check), expected_hash=expected_hash,
            actor=actor, updated_at=updated_at
        )
    ...
    def revise_task(
        ...
        return self._write_definition(
            entity_kind="task", entity_id=task.id, body=_task_body(task), expected_hash=expected_hash,
            actor=actor, updated_at=updated_at
        )
    ...
    def definition_history(
        ...
        return tuple(
            DefinitionRevision(
                ...
                version=int(row["version"]), body=json.loads(str(row["body_json"])),
                body_sha256=str(row["body_sha256"]), actor=str(row["actor"]),
    ...
    def append_event(self, event: Event) -> int:
        with self._connect() as connection:
            cursor = connection.execute(
                ...
                """,
                (
                    event.type,event.ts.isoformat(),event.event_id,event.session_id,event.device_id,
                    event.client_ts.isoformat() if event.client_ts is not None else None,
                    event.room,event.task_id,event.check_id,
                    event.answer.value if event.answer is not None else None,
                    event.energy,event.time_min,event.hand_size,
    ...
    def checks(self) -> tuple[Check, ...]:
        with self._connect() as connection:
            rows = connection.execute(
                "SELECT id,room,prompt,linked_task_ids_json,trigger_answer FROM checks ORDER BY room,id"
            ).fetchall()
        return tuple(
            Check(
                id=str(r["id"]), room=str(r["room"]), prompt=str(r["prompt"]),
                linked_task_ids=tuple(json.loads(str(r["linked_task_ids_json"]))),
                trigger_answer=Answer(str(r["trigger_answer"]))
                if r["trigger_answer"] is not None else None,
            )
            for r in rows
        )
    ...
    def tasks(self) -> tuple[Task, ...]:
        with self._connect() as connection:
            rows = connection.execute(
                "SELECT id,title,room,kind,effort,minutes_est,frequency_days FROM tasks ORDER BY room,title,id"
            ).fetchall()
        return tuple(
            Task(
                id=str(r["id"]), title=str(r["title"]), room=str(r["room"]), kind=str(r["kind"]),
                effort=int(r["effort"]), minutes_est=int(r["minutes_est"]),
    ...
    def events(self) -> tuple[Event, ...]:
        ...
        return tuple(
            Event(
                type=str(r["type"]), ts=datetime.fromisoformat(str(r["ts"])),
                ...
                client_ts=datetime.fromisoformat(str(r["client_ts"])) if r["client_ts"] is not None else None,
    ...
    def _event_body(event: Event) -> dict[str, Any]:
        return {
            "type": event.type, "ts": event.ts.isoformat(), "event_id": event.event_id,
            "session_id": event.session_id, "device_id": event.device_id,
            "client_ts": event.client_ts.isoformat() if event.client_ts is not None else None,
            "room": event.room, "task_id": event.task_id, "check_id": event.check_id,
            "answer": event.answer.value if event.answer is not None else None,
            "energy": event.energy, "time_min": event.time_min, "hand_size": event.hand_size,
    ...
    def export_bundle(self) -> dict[str, object]:
        ...
        return {
            ...
            "checks": [
                {**asdict(check), "trigger_answer": check.trigger_answer.value if check.trigger_answer else None}
            ...
            "definition_revisions": [
                {
                    "entity_kind": revision.entity_kind, "entity_id": revision.entity_id,
                    "version": revision.version, "body": revision.body,
                    "body_sha256": revision.body_sha256, "actor": revision.actor,
                    "updated_at": revision.updated_at.isoformat(),
                }
                for kind, entity_id in identities
    ...
    def replace_from_bundle(self, bundle: dict[str, object]) -> None:
        ...
        checks = [
            Check(
                id=item["id"], room=item["room"], prompt=item["prompt"],
                linked_task_ids=tuple(item["linked_task_ids"]),
                trigger_answer=Answer(item["trigger_answer"])
                if item.get("trigger_answer") is not None else None,
            )
        ...
        events = [
            Event(
                type=item["type"], ts=datetime.fromisoformat(item["ts"]),
                event_id=item.get("event_id"), session_id=item.get("session_id"),
                device_id=item.get("device_id"),
                client_ts=datetime.fromisoformat(item["client_ts"])
                if item.get("client_ts") is not None else None,
                room=item.get("room"), task_id=item.get("task_id"), check_id=item.get("check_id"),
                answer=Answer(item["answer"]) if item.get("answer") is not None else None,
                energy=item.get("energy"), time_min=item.get("time_min"), hand_size=item.get("hand_size"),
        ...
        with self._connect() as connection:
            ...
            for check in checks:
                connection.execute(
                    "INSERT INTO checks(id,room,prompt,linked_task_ids_json,trigger_answer) VALUES(?,?,?,?,?)",
                    (
                        check.id, check.room, check.prompt,
                        json.dumps(list(check.linked_task_ids), separators=(",", ":")),
                        check.trigger_answer.value if check.trigger_answer is not None else None,
                    ),
                )
            for task in tasks:
                connection.execute(
                    "INSERT INTO tasks(id,title,room,kind,effort,minutes_est,frequency_days) VALUES(?,?,?,?,?,?,?)",
                    (task.id,task.title,task.room,task.kind,task.effort,task.minutes_est,task.frequency_days),
                )
            for event in events:
                ...
                connection.execute(
                    ...
                    """,
                    (
                        body["type"],body["ts"],body["event_id"],body["session_id"],body["device_id"],
                        body["client_ts"],body["room"],body["task_id"],body["check_id"],body["answer"],
                        body["energy"],body["time_min"],body["hand_size"],
                        json.dumps(body["task_ids"],separators=(",", ":")),
                    ),
                )
            if revisions:
                for revision in revisions:
                    body = dict(revision["body"])
                    if _body_hash(body) != revision["body_sha256"]:
                        raise StoreError(
                            f"definition revision hash mismatch for {revision['entity_kind']}:{revision['entity_id']}"
                        )
                    self._insert_revision(
                        connection, str(revision["entity_kind"]), str(revision["entity_id"]),
                        int(revision["version"]), body, str(revision["actor"]),
                        str(revision["updated_at"]),
                    )
            else:
                self._seed_definition_revisions(connection, actor="restore:legacy")
